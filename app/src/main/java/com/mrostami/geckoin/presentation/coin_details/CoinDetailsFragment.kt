...
class CoinDetailsFragment : Fragment(R.layout.coin_details_fragment) {
    ...
    val navargs: CoinDetailsFragmentArgs by navArgs()
    ...
    private fun updateCoinInfo(coinInfo: CoinDetailsInfo?) {
        ...
        with(binding) {
            ...
            coinInfo.description?.let { description ->
                txtDescription.text = HtmlCompat.fromHtml("<h2>About ${coinInfo.name}</h2><br><p>${coinInfo.description}</p>", HtmlCompat.FROM_HTML_MODE_COMPACT)
            }
            txtCoinName.text = coinInfo.name
            txtUsdMarketCap.text = "# ${coinInfo.marketCapRank}"
//            txtCoinSymbol.text = coin.symbol
//            txtCoinPrice.text = coinInfo.priceBtc?.toString() + "(btc)"
        }
    }
}