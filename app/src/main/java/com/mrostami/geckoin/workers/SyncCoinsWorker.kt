...
import com.mrostami.geckoin.domain.usecases.SyncAllCoinsUseCase
    ...
    override suspend fun doWork(): Result = withContext(Dispatchers.IO) {
        ...
        try {
            ...
            syncAllCoinsUseCase.invoke(forceRefresh = false).collectLatest { result ->
                when (result) {
                    is com.mrostami.geckoin.domain.base.Result.Success -> {
                        workResult = Result.success()
                    }
//                    is com.mrostami.geckoin.domain.base.Result.Error -> {
